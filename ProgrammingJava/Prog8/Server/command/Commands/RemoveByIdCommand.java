...
public class RemoveByIdCommand extends Command {
     ...
     */
    public RemoveByIdCommand(CollectionManager manager, CollectionDatabaseManager cdm){
        super("remove_id");
        ...
        try {
            ...
            if (manager.containsId(id)) {
                if(cdm.isOwner(id, userData)) {
                    ...
                    printStream.println("Билет с ID = " + id + " был успешно удален.");
                    userData.setSuccess(true);
                }else{
                    printStream.println("Вы не имеете право удалять данный билет");
                }
            } else printStream.println("Билета с указанным ID уже не существует.");
        }catch (Exception e) {
            e.printStackTrace();
        }
        finally {
