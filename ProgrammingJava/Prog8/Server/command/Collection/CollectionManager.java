...
public class CollectionManager {
    ...
    public CollectionManager(TreeMap<Integer,Ticket> treeMap){
    ...
    public void removeById(Long id){
        ...
        treeMap.entrySet().stream().filter(mapEntry -> mapEntry.getValue().getId().equals(id))
    ...
    public Long getIdByKey(Integer key){
        if(treeMap.containsKey(key)){
            return treeMap.get(key).getId();
        }else{
            System.out.println("Невозможно получить id по key: билета с указанным ключом не существует.");
            return null;
        }
    }
    /**
     ...
     * @param key - ключ.
     */
    public Long[] removeGreaterKey(Integer key, PrintStream printStream){
        if(treeMap.size() == 0){
            printStream.println("Коллекция пуста");
            return null;
        }else {
            ...
            treeMap.entrySet().stream().filter(mapEntry -> mapEntry.getKey() > key)
                    .forEach(mapEntry -> keys.add(mapEntry.getKey()));
            return keys.stream().map(k -> treeMap.get(k).getId()).toList().toArray(new Long[0]);
        }
    }
    /**
     ...
     */
    public Long[] removeGreater(Ticket ticket, PrintStream printStream){
        if (treeMap.size() == 0){
            printStream.println("Коллекция пуста");
            return null;
        } else{
